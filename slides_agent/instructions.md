...
3.  **Research (Two-Stage Approach)**:
    ...
    *   *Hard Guard*: Do **not** call ProgrammaticTool/IPythonInterpreter/PersistentShellTool for web fetching. Use the facts and content provided in the task brief — the model running this agent does not have web search access.
...
**Image sourcing rule (critical):** Never construct, guess, or recall image URLs from memory. Every URL passed to `DownloadImage` must come directly from a tool result — use `ImageSearch`.
