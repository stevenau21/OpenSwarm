...
2. **Do not attempt web research** — web search tools are not available in this configuration. If the document requires facts, statistics, or up-to-date information, use the model's training knowledge and note that data may not be current.
...
4. **Generate Content**: Choose HTML or Markdown
   ...
   - Markdown: Plain text structure only (no DOCX/PDF generation)
     ...
     - Image sourcing is not available in this configuration — skip visuals or note they are unavailable
